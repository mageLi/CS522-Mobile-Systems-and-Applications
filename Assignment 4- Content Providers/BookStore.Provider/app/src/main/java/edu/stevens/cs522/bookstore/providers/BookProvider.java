...
public class BookProvider extends ContentProvider {
    ...
    private static final String AUTHORS_TABLE = "author";

    private static final String CREATE_BOOKS_TABLE = "create table " + BOOKS_TABLE + " (" + BookContract.ID + " integer primary key autoincrement," +
            BookContract.TITLE + " text," + BookContract.AUTHORS + " text," + BookContract.ISBN + " text," + BookContract.PRICE + " text);";

    private static final String CREATE_AUTHORS_TABLE = "create table " + AUTHORS_TABLE + " (" + AuthorContract.ID + " integer primary key autoincrement," +
            AuthorContract.FIRST_NAME + " text," + AuthorContract.MIDDLE_INITIAL + " text," + AuthorContract.LAST_NAME + " text," + AuthorContract.BOOK_FK +
            " integer not null,foreign key (" + AuthorContract.BOOK_FK + ") references " + BOOKS_TABLE + "(" + BookContract.ID + ")on delete cascade );";

    private static final String DROP_BOOK_TABLE = "DROP TABLE IF EXISTS " + BOOKS_TABLE;

    private static final String DROP_AUTHORS_TABLE = "DROP TABLE IF EXISTS " + BOOKS_TABLE;

    private static HashMap<String, String> columnMap = new HashMap<>();

    public static String CREATE_INDEX = "Create index AuthorsBookIndex on " + AUTHORS_TABLE + "(" + AuthorContract.BOOK_FK + ");";
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (uriMatcher.match(uri)) {
            case ALL_ROWS:
                ...
                long rowId = db.insert(BOOKS_TABLE, null, values);
                if (rowId > 0) {
                    Uri _uri = ContentUris.withAppendedId(BookContract.CONTENT_URI, rowId);
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        switch (uriMatcher.match(uri)) {
            case ALL_ROWS:
                ...
                String inTables = BOOKS_TABLE
                        + " LEFT OUTER JOIN " + AUTHORS_TABLE
                        + " ON (" + BOOKS_TABLE + "." + BookContract.ID
                        + " = " + AUTHORS_TABLE + "." + AuthorContract.BOOK_FK + ")";
                liteQueryBuilder.setTables(inTables);
                for (String field :
                        projection) {
                    if (!columnMap.containsKey(field)) columnMap.put(field, field);
                }
                liteQueryBuilder.setProjectionMap(columnMap);
                break;
            ...
            case SINGLE_ROW:
                ...
                liteQueryBuilder.setTables(BOOKS_TABLE);
                liteQueryBuilder.appendWhere(BookContract.ID + " = " + BookContract.getId(uri));
                break;
                //throw new UnsupportedOperationException("Not yet implemented");
            default:
                throw new IllegalStateException("insert: bad case");
        }
        Cursor cursor = liteQueryBuilder.query(db, projection, selection, selectionArgs, null, null, null);
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (uriMatcher.match(uri)) {
            case ALL_ROWS:
                selectId = db.delete(BOOKS_TABLE, selection, selectionArgs);
                db.delete(AUTHORS_TABLE, selection, selectionArgs);
                getContext().getContentResolver().notifyChange(uri, null);
                break;
            case SINGLE_ROW:
                String id = uri.getPathSegments().get(1);
                Log.i("delete", "Delete_id = " + id);
                selectId = db.delete(BOOKS_TABLE, BOOKS_TABLE + "." + BookContract.ID + " = " + Integer.parseInt(id) + (!TextUtils.isEmpty(selection) ? " AND (" + selection + ')' : ""), selectionArgs);
                db.delete(AUTHORS_TABLE, AUTHORS_TABLE + "." + AuthorContract.BOOK_FK + " = " + id + (!TextUtils.isEmpty(selection) ? " AND (" + selection + ')' : ""), selectionArgs);
                getContext().getContentResolver().notifyChange(uri, null);
                break;
            default:
                throw new UnsupportedOperationException("Not yet implemented");
